...
public class WeaknessMitigation extends SCBaseSkillPlugin {
    ...
    public void addTooltip(SCData data, TooltipMakerAPI tooltip) {
        ...
        tooltip.addPara("%s damage taken by weapons (%s × skill efficiency)", 10f, Misc.getHighlightColor(), Misc.getHighlightColor(),
                "-" + Math.round(eData.totalMult * WEAPON_DMG_TAKEN_MULT * 100f) + "%", Math.round(WEAPON_DMG_TAKEN_MULT * 100f) + "%");
        tooltip.addPara("%s damage taken by engines (%s × skill efficiency)", 0f, Misc.getHighlightColor(), Misc.getHighlightColor(),
                "-" + Math.round(eData.totalMult * ENGINE_DMG_TAKEN_MULT * 100f) + "%", Math.round(ENGINE_DMG_TAKEN_MULT * 100f) + "%");
        tooltip.addPara("%s damage taken by emp (%s × skill efficiency)", 0f, Misc.getHighlightColor(), Misc.getHighlightColor(),
                "-" + Math.round(eData.totalMult * EMP_DMG_TAKEN_MULT * 100f) + "%", Math.round(EMP_DMG_TAKEN_MULT * 100f) + "%");
    ...
    public void applyEffectsBeforeShipCreation(SCData data, MutableShipStatsAPI stats, ShipVariantAPI variant, ShipAPI.HullSize hullSize, String id) {
        ...
        stats.getWeaponDamageTakenMult().modifyMult(getId(), 1f - (eData.totalMult * WEAPON_DMG_TAKEN_MULT));
        stats.getEmpDamageTakenMult().modifyMult(getId(), 1f - (eData.totalMult * ENGINE_DMG_TAKEN_MULT));
        stats.getEngineDamageTakenMult().modifyMult(getId(), 1f - (eData.totalMult * EMP_DMG_TAKEN_MULT));
    }
}
