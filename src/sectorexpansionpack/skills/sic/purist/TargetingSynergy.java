...
public class TargetingSynergy extends SCBaseSkillPlugin {
    ...
    public void addTooltip(SCData data, TooltipMakerAPI tooltip) {
        ...
        tooltip.addPara("%s purist skill efficiency*", 0f, Misc.getHighlightColor(), Misc.getHighlightColor(), Math.round(pData.totalMult * 100f) + "%");
    ...
    public void applyEffectsBeforeShipCreation(SCData data, MutableShipStatsAPI stats, ShipVariantAPI variant, ShipAPI.HullSize hullSize, String id) {
        ...
        if (Objects.equals(variantType, pData.primary) || (pData.hasDesignCompromise && Objects.equals(variantType, pData.secondary))) {
            stats.getBallisticWeaponRangeBonus().modifyMult(getId(), 1f + (pData.totalMult * NON_MISSILE_WEAPON_RANGE_MULT));
            stats.getBallisticWeaponDamageMult().modifyMult(getId(), 1f + (pData.totalMult * NON_MISSILE_WEAPON_DAMAGE_MULT));

            stats.getEnergyWeaponRangeBonus().modifyMult(getId(), 1f + (pData.totalMult * NON_MISSILE_WEAPON_RANGE_MULT));
            stats.getEnergyWeaponDamageMult().modifyMult(getId(), 1f + (pData.totalMult * NON_MISSILE_WEAPON_DAMAGE_MULT));
        }
    }
}
