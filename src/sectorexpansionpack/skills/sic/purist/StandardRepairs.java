...
public class StandardRepairs extends SCBaseSkillPlugin {
    ...
    public void addTooltip(SCData data, TooltipMakerAPI tooltip) {
        ...
        tooltip.addPara("%s (Max: %s) hull and armor repair rate outside of combat", 10f, Misc.getHighlightColor(), Misc.getHighlightColor(), "+" + Math.round(REPAIR_RATE_PER_DAY_MULT * debuffMult * 100f) + "%", Math.round(REPAIR_RATE_PER_DAY_MULT * 100f) + "%");
        tooltip.addPara("%s (Max: %s) flat increase to combat readiness recovered per day", 0f, Misc.getHighlightColor(), Misc.getHighlightColor(), "+" + Math.round(CR_RECOVERY_RATE_PER_DAY_MOD * debuffMult * 100f) + "%", Math.round(CR_RECOVERY_RATE_PER_DAY_MOD * 100f) + "%");
    }
    ...
    public void applyEffectsBeforeShipCreation(SCData data, MutableShipStatsAPI stats, ShipVariantAPI variant, ShipAPI.HullSize hullSize, String id) {
        ...
        if (Objects.equals(variantType, primaryType) || (hasDesignCompromise && Objects.equals(variantType, secondaryType))) {
            ...
            stats.getRepairRatePercentPerDay().modifyPercent(id, REPAIR_RATE_PER_DAY_MULT * debuffMult);
            stats.getBaseCRRecoveryRatePercentPerDay().modifyFlat(id, CR_RECOVERY_RATE_PER_DAY_MOD * debuffMult);
        }
    }
}
